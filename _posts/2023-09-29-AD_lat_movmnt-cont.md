...
mayber we logged into a local account, perhaps system create our ghoog account but we need to get on the domain

if we have creds we can enter

    runas /user:oscp\web_svc powershell.exe

from here we can copy over PowerView.ps1 and run a few useful commands to enumerate users, computers, shares, and if any object has permissions that are juicy